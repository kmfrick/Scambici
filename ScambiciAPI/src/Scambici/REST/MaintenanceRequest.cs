...
{
	public class FormMaintenanceRequest : Scambici.Domain.UserMaintenance
	{
		public int UserId
		{ get; set; }
	...
	{
		[FunctionName("MaintenanceRequest")]
				...
				ILogger log)
		{
			...
			var requestData = JsonConvert.DeserializeObject<FormMaintenanceRequest>(requestBody);
			try
			{
				using var usersController = new UsersControllerAzure();
				var user = usersController.GetUserById(requestData.UserId);
				var faultDescription = requestData.FaultDescription;
				using var maintReqController = new MaintenanceRequestControllerAzure(user, requestData.Timestamp, requestData.Signature);
				maintReqController.RequestMaintenance(user, faultDescription);
