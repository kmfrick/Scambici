...
namespace Scambici.REST
{
	...
	public static class UserDeletion
	{
				...
				ILogger log)
		{
			...
			var requestData = JsonConvert.DeserializeObject<FormUserDeletion>(requestBody);
			using var usersController = new UsersControllerAzure();
			var user = usersController.GetUserById(requestData.UserId);
			try
			{
				using var controller = new UserDeletionControllerAzure(user, requestData.Timestamp, requestData.Signature);
