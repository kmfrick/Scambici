...
namespace Scambici.REST
{
	...
	public static class BikeScrap
	{
		[FunctionName("BikeScrap")]
				...
				ILogger log)
		{
			...
			var requestData = JsonConvert.DeserializeObject<FormBikeScrap>(requestBody);
			try
			{
				...
				var store = storesController.GetStoreByEmployee(employee);
				var bikeId = ((Scambici.Domain.StorageMaintenance) employee.CurrentTask).Bike.BikeId;
				storageMaintController.ScrapBike(store, bikeId);
