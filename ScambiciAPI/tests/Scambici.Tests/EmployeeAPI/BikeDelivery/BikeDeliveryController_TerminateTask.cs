...
{
	public class TerminateTaskControllerTestable : Scambici.EmployeeAPI.BikeDelivery.BikeDeliveryController 
	{
		public TerminateTaskControllerTestable(Scambici.Domain.ScambiciContext context)
		{	dbContext = context; }
	}

	[NUnit.Framework.TestFixture]
	public class BikeDeliveryController_TerminateTask
	{
		...
		public void SetUp()
		{
			...
			controller = new TerminateTaskControllerTestable(dbContext);
			//Employee
		...
		public void BikeDeliveryController_DBHasOperativeStore_TerminateTask()
		{
			var employee = dbContext.Employees.First();
			var user = dbContext.Users.First();
			//Take task
			controller.TakeTask(employee, 2);
			//Complete task
			NUnit.Framework.Assert.That(() => controller.TerminateTask(employee, 5566), NUnit.Framework.Throws.Nothing);
			NUnit.Framework.Assert.That(employee.CurrentTask, NUnit.Framework.Is.EqualTo(null));
			NUnit.Framework.Assert.That(employee.Tasks.First().TaskId, NUnit.Framework.Is.EqualTo(2));
